...
export default function DepositPage({ onBack, onViewHistory, balance, user, playSound }: { onBack: () => void, onViewHistory: () => void, balance: number, user: any, playSound: (type: any) => void }) {
  ...
  const handleNextStep = () => {
    ...
    const maxLimit = adminPaymentMethods?.[method]?.maxDeposit || (method === 'bank' ? 1000000 : 10000);
    ...
    if (numAmount > maxLimit) {
      alert(`Maximum deposit for ${method === 'easypaisa' ? 'Easypaisa' : method === 'jazzcash' ? 'JazzCash' : 'Bank Transfer'} is Rs ${maxLimit.toLocaleString()}.`);
      return;
    }
    setStep(2);
  };
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 gap-3">
                  {paymentMethods.map((pm) => (
                    ...
                    >
                      <div className={cn("w-12 h-12 rounded-xl flex items-center justify-center shadow-sm overflow-hidden", pm.bgColor)}>
                        <pm.icon size={24} className={pm.color} />
                      </div>
                      <div className="flex-1 text-left">
                        ...
                        <div className="text-[9px] text-gray-400 font-bold uppercase tracking-tighter">
                          Max Limit: Rs {adminPaymentMethods?.[pm.id]?.maxDeposit?.toLocaleString() || (pm.id === 'bank' ? '1,000,000' : '10,000')}
                        </div>
                      </div>
                      <div className={cn(
            ...
            <>
              <div className="mb-6">
                ...
                <div className="bg-gray-50 p-5 rounded-2xl border-2 border-gray-100 space-y-3">
                  {adminPaymentMethods?.[method]?.qrCodeBase64 && (
                    <div className="flex flex-col items-center mb-4">
                      <span className="text-[10px] text-gray-400 font-black uppercase tracking-widest mb-2">Scan QR Code</span>
                      <div className="w-48 h-48 bg-white p-2 rounded-xl border-2 border-gray-100 shadow-sm">
                        <img src={adminPaymentMethods[method].qrCodeBase64} alt="QR Code" className="w-full h-full object-contain" />
                      </div>
                    </div>
                  )}
                  
                  {adminPaymentMethods?.[method]?.instructions && (
                  ...
                  {adminPaymentMethods?.[method]?.accountName && (
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] text-gray-400 font-black uppercase tracking-tighter">Account Name</span>
                      <span className="text-xs font-black text-gray-800">{adminPaymentMethods[method].accountName}</span>
                    </div>
                  )}
                  {adminPaymentMethods?.[method]?.accountNumber && (
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] text-gray-400 font-black uppercase tracking-tighter">Account Number</span>
                      <span className="text-xs font-black text-gray-800 tracking-wider">{adminPaymentMethods[method].accountNumber}</span>
                    </div>
                  )}
                  {method === 'bank' && adminPaymentMethods?.bank?.bankName && (
                    <div className="flex justify-between items-center">
                      <span className="text-[10px] text-gray-400 font-black uppercase tracking-tighter">Bank Name</span>
                      <span className="text-xs font-black text-gray-800">{adminPaymentMethods.bank.bankName}</span>
                    </div>
                  )}

...
export default function WithdrawalPage({ onBack, onViewHistory, balance, user, playSound, minWithdrawalLimit = 200 }: { onBack: () => void, onViewHistory: () => void, balance: number, user: any, playSound: (type: 'bet' | 'win' | 'loss' | 'notify' | 'click') => void, minWithdrawalLimit?: number }) {
  ...
  const handleWithdraw = async () => {
    ...
    if (parseFloat(amount) < minWithdrawalLimit) {
      setError(`Minimum withdrawal amount is Rs ${minWithdrawalLimit}`);
      return;
    }
    ...
    try {
      ...
      const details = method === 'bank' 
        ? `Bank: ${bankName}, Name: ${accountName}, A/C: ${accountNumber}`
        : `${method === 'easypaisa' ? 'Easypaisa' : 'Jazzcash'} - Name: ${accountName}, Number: ${accountNumber}`;
  ...
  return (
    ...
    >
      {/* Header */}
      <div className="bg-gradient-to-b from-[#00c853] to-[#00b24a] pt-6 px-6 pb-20 text-white flex flex-col items-center relative rounded-b-[3rem] shadow-2xl overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <div className="text-sm opacity-90 font-bold uppercase tracking-[0.2em] mb-1">Available Balance</div>
          <div className="text-5xl font-black drop-shadow-sm leading-tight">Rs {balance.toFixed(2)}</div>
        </div>
      </div>
      ...
      <div className="px-4 -mt-8 relative z-10 space-y-6">
        ...
        <div className="bg-white rounded-[2.5rem] p-8 shadow-2xl border border-gray-50">
          ...
          {/* Method Selection */}
          <div className="mb-8">
            <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4 ml-1">Select Account Type</label>
          ...
          <div className="mb-8 space-y-4">
            <label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 ml-1">Account Information</label>
            ...
            <div className="relative">
              <input 
                ...
                className="w-full px-6 py-4 rounded-2xl bg-gray-50 border-2 border-transparent focus:border-[#00c853] focus:bg-white transition-all outline-none font-bold text-gray-700 placeholder:text-gray-300 shadow-inner"
            ...
            <div className="relative">
              <input 
                ...
                className="w-full px-6 py-4 rounded-2xl bg-gray-50 border-2 border-transparent focus:border-[#00c853] focus:bg-white transition-all outline-none font-bold text-gray-700 placeholder:text-gray-300 shadow-inner"
                placeholder={method === 'bank' ? "Account Number / IBAN" : "Mobile Account Number"}
              />
            </div>
          </div>
        ...
        <div className="bg-white rounded-[2.5rem] p-8 shadow-xl border border-gray-50">
          ...
          <div className="space-y-4">
            {[
              ...
              { text: `Minimum withdrawal amount is Rs ${minWithdrawalLimit}.`, icon: "💰" },

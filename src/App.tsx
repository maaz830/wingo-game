...
function AppContent() {
  const [view, setView] = useState<'landing' | 'game' | 'login' | 'register' | 'admin' | 'wallet' | 'referral' | 'stats' | 'deposit' | 'withdraw' | 'withdrawalHistory' | 'account' | 'gameHistory' | 'transactionHistory'>('landing');
    ...
    if (user?.uid) {
      ...
      const unsubscribe = onSnapshot(doc(db, 'users', user.uid), (snapshot) => {
        if (snapshot.exists()) {
          ...
          const finalRole = user.email === 'dirtcalculator@gmail.com' ? 'admin' : (userData.role || 'user');
  ...
  useEffect(() => {
    const unsubscribe = onSnapshot(doc(db, 'settings', 'global'), (snapshot) => {
      if (snapshot.exists()) {
        ...
        setMinWithdrawal(data.minWithdrawal ?? 200);
      }
    }, (error) => {
      console.warn("Error fetching global settings in AppContent:", error);
    });
    return () => unsubscribe();
  }, []);
        ...
        try {
          ...
          if (cachedUser) {
            ...
            if (userDoc.exists()) {
              ...
              const finalRole = firebaseUser.email === 'dirtcalculator@gmail.com' ? 'admin' : (userData.role || 'user');
              ...
              const newUser = {
                ...
                balance: 0,
                role: firebaseUser.email === 'dirtcalculator@gmail.com' ? 'admin' : 'user',
                isBlocked: false,
          ...
          const finalRole = firebaseUser.email === 'dirtcalculator@gmail.com' ? 'admin' : 'user';
  ...
  if (view === 'login') {
    return <LoginPage onBack={() => setView('landing')} onRegister={() => setView('register')} onLoginSuccess={(u) => { 
      const finalRole = u.email === 'dirtcalculator@gmail.com' ? 'admin' : (u.role || 'user');
  ...
  if (view === 'withdraw') {
    return <WithdrawalPage onBack={() => setView('wallet')} onViewHistory={() => setView('withdrawalHistory')} balance={balance} user={user} playSound={playSound} minWithdrawalLimit={minWithdrawal} />;
  }
  ...
  if (view === 'account') {
    return <AccountPage onBack={() => setView('game')} user={user} balance={balance} setView={setView} onLogout={() => signOut(auth)} />;
  }

  if (view === 'gameHistory') {
    return <GameHistoryPage onBack={() => setView('account')} user={user} />;
  }

  if (view === 'transactionHistory') {
    return <TransactionHistoryPage onBack={() => setView('account')} user={user} />;
  }
  ...
  return (
    <div className="min-h-screen bg-gray-100 font-sans text-gray-900 pb-20">
          ...
          >
            ...
            >
              ...
              <div className="space-y-6">
                ...
                <div className="bg-gray-50 rounded-2xl p-4 flex justify-between items-center mb-2">
                ...
                <div className="bg-green-50 rounded-2xl p-4 flex justify-between items-center mb-4 border border-green-100">
                  <div className="text-xs text-green-600 font-bold uppercase tracking-wider">Potential Win</div>
                  <div className="text-xl font-black text-green-600">
                    Rs {(
                      (betAmount * selectedMultiplier) * 
                      (betSelection.type === 'number' ? 8.1 : betSelection.value === 'violet' ? 4.05 : 1.8)
                    ).toFixed(2)}
                  </div>
                </div>
                  ...
                  <div className="bg-gray-100 p-4 rounded-xl mb-4 border border-gray-200">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-sm font-black text-green-600">
                        Rs {((autoBetConfig.amount * autoBetConfig.multiplier) * (autoBetConfig.type === 'number' ? 8.1 : autoBetConfig.value === 'violet' ? 4.05 : 1.8)).toFixed(2)}
                      </span>
                    </div>
                  </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 p-2 flex justify-around items-center shadow-2xl z-50">
        ...
        <div 
          className={cn("flex flex-col items-center cursor-pointer", view === 'account' ? "text-green-600" : "text-gray-400")}
...
function LandingPage({ onStart, onLogin, onRegister }: { onStart: () => void, onLogin: () => void, onRegister: () => void }) {
  ...
  useEffect(() => {
    ...
    // Fetch video HTML from global settings
    const unsubscribe = onSnapshot(doc(db, 'settings', 'global'), (snapshot) => {
      if (snapshot.exists()) {
        setVideoHtml(snapshot.data().howToPlayVideoHtml || null);
      }
    });
  ...
  return (
    <div className="min-h-screen bg-[#0a0a0a] text-white overflow-x-hidden">
      ...
      <section className="px-6 py-20 max-w-6xl mx-auto">
        ...
        <div className="relative aspect-video rounded-3xl overflow-hidden border border-white/10 shadow-2xl group flex items-center justify-center bg-zinc-900/50">
          {videoHtml ? (
            <div 
    ...
    try {
      ...
      if (!userDoc.exists()) {
        userData = {
          ...
          email: user.email,
          balance: 0,
          role: user.email === 'dirtcalculator@gmail.com' ? 'admin' : 'user',
          createdAt: new Date().toISOString()
        };
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5] flex flex-col">
      <div className="bg-gradient-to-b from-[#00c853] to-[#00e676] p-6 text-white relative">
        ...
        <p className="text-sm opacity-90 text-center">Sign in securely with your Google account</p>
      </div>
      ...
      <div className="flex-1 bg-white rounded-t-[32px] -mt-6 p-6 shadow-xl">
        ...
        <div className="space-y-6">
          <div className="text-center py-4">
            <p className="text-gray-500 text-sm font-bold mb-6">Quick and secure access to your account</p>
            <button 
              onClick={handleGoogleLogin}
              disabled={loading}
              className="w-full bg-white border-2 border-gray-100 text-gray-700 py-4 rounded-2xl font-black flex items-center justify-center gap-3 hover:bg-gray-50 transition-all active:scale-95 disabled:opacity-50 shadow-sm"
            >
          ...
          <div className="relative flex items-center py-4">
            ...
            <span className="flex-shrink mx-4 text-gray-400 text-[10px] font-black uppercase tracking-widest">Account Options</span>
            <div className="flex-grow border-t border-gray-100"></div>
          </div>

          <button 
            onClick={onRegister}
            className="w-full border-2 border-[#00c853] text-[#00c853] py-4 rounded-full font-black text-lg active:scale-95 transition-all uppercase tracking-widest"
          >
            Create New Account
          </button>
          ...
          <div className="flex justify-around pt-8">
            <div className="flex flex-col items-center gap-2">
              ...
              <span className="text-[10px] text-gray-500 font-black uppercase tracking-widest">Forgot password</span>
            </div>
            <div className="flex flex-col items-center gap-2">
              <div className="w-12 h-12 bg-white rounded-full shadow-md flex items-center justify-center text-[#00c853]">
                <Bell size={24} />
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      if (error.code === 'auth/operation-not-allowed') {
        ...
        setError("Registration cancelled. Please complete the sign-in in the popup.");
      } else if (error.code === 'auth/cancelled-popup-request') {
        setError("Registration request was cancelled.");
      } else if (error.code === 'auth/popup-blocked') {
        setError("Registration popup was blocked by your browser. Please allow popups for this site.");
      } else {
        setError("Google login failed. Please try again.");
      }
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#f5f5f5] flex flex-col">
      <div className="bg-gradient-to-b from-[#00c853] to-[#00e676] p-6 text-white relative">
        ...
        <p className="text-sm opacity-90 text-center">Join us instantly with your Google account</p>
      </div>
      ...
      <div className="flex-1 bg-white rounded-t-[32px] -mt-6 p-6 shadow-xl">
        ...
        <div className="space-y-5">
          <div className="text-center py-4">
            <p className="text-gray-500 text-sm font-bold mb-6">Join us instantly using your Google account</p>
            <button 
              onClick={handleGoogleLogin}
              disabled={loading}
              className="w-full bg-white border-2 border-gray-100 text-gray-700 py-4 rounded-2xl font-black flex items-center justify-center gap-3 hover:bg-gray-50 transition-all active:scale-95 disabled:opacity-50 shadow-sm"
            >
              <img src="https://www.google.com/favicon.ico" alt="Google" className="w-6 h-6" />
              <span className="uppercase tracking-widest text-sm">Register with Google</span>
            </button>
          </div>

          <div className="relative flex items-center py-4">
            <div className="flex-grow border-t border-gray-100"></div>
            <span className="flex-shrink mx-4 text-gray-400 text-[10px] font-black uppercase tracking-widest">Already a member?</span>
            <div className="flex-grow border-t border-gray-100"></div>
          </div>
...
function AdminPanel({ onBack }: { onBack: () => void }) {
  ...
  const [howToPlayVideoHtml, setHowToPlayVideoHtml] = useState("");
  const [minWithdrawal, setMinWithdrawal] = useState(200);
  ...
  const [jazzCashDetails, setJazzCashDetails] = useState({ accountName: '', accountNumber: '', qrCodeBase64: '', instructions: '', maxDeposit: 10000 });
  const [easypaisaDetails, setEasypaisaDetails] = useState({ accountName: '', accountNumber: '', qrCodeBase64: '', instructions: '', maxDeposit: 10000 });
  const [bankDetails, setBankDetails] = useState({ accountName: '', accountNumber: '', bankName: '', qrCodeBase64: '', instructions: '', maxDeposit: 100000 });
  ...
  useEffect(() => {
    ...
    if (cachedGlobal) {
      ...
      setHowToPlayVideoHtml(cachedGlobal.howToPlayVideoHtml || "");
      setMinWithdrawal(cachedGlobal.minWithdrawal ?? 200);
      setWinProbability(cachedGlobal.winProbability ?? 50);
    ...
    const unsubscribe = onSnapshot(doc(db, 'settings', 'global'), (snapshot) => {
      if (snapshot.exists()) {
        ...
        setHowToPlayVideoHtml(data.howToPlayVideoHtml || "");
        setMinWithdrawal(data.minWithdrawal ?? 200);
        setWinProbability(data.winProbability ?? 50);
  ...
  useEffect(() => {
    ...
    if (cachedPayment) {
      setJazzCashDetails(cachedPayment.jazzcash || { accountName: '', accountNumber: '', qrCodeBase64: '', instructions: '', maxDeposit: 10000 });
      setEasypaisaDetails(cachedPayment.easypaisa || { accountName: '', accountNumber: '', qrCodeBase64: '', instructions: '', maxDeposit: 10000 });
      setBankDetails(cachedPayment.bank || { accountName: '', accountNumber: '', bankName: '', qrCodeBase64: '', instructions: '', maxDeposit: 100000 });
    }
    ...
    const unsubscribe = onSnapshot(doc(db, 'settings', 'paymentMethods'), (snapshot) => {
      if (snapshot.exists()) {
        ...
        setJazzCashDetails(data.jazzcash || { accountName: '', accountNumber: '', qrCodeBase64: '', instructions: '', maxDeposit: 10000 });
        setEasypaisaDetails(data.easypaisa || { accountName: '', accountNumber: '', qrCodeBase64: '', instructions: '', maxDeposit: 10000 });
        setBankDetails(data.bank || { accountName: '', accountNumber: '', bankName: '', qrCodeBase64: '', instructions: '', maxDeposit: 100000 });
      }
    }, (error) => {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6 pb-24">
      ...
      <div className="bg-white rounded-[32px] shadow-sm border border-gray-200 p-8 mb-8">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-6">
          ...
          <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
            ...
            <input type="text" placeholder="Account Number" value={jazzCashDetails.accountNumber} onChange={e => setJazzCashDetails({...jazzCashDetails, accountNumber: e.target.value})} className="w-full mb-2 p-2 text-xs rounded border border-gray-200" />
            <input type="number" placeholder="Max Deposit Limit" value={jazzCashDetails.maxDeposit} onChange={e => setJazzCashDetails({...jazzCashDetails, maxDeposit: Number(e.target.value)})} className="w-full mb-2 p-2 text-xs rounded border border-gray-200" />
            <textarea placeholder="Instructions" value={jazzCashDetails.instructions} onChange={e => setJazzCashDetails({...jazzCashDetails, instructions: e.target.value})} className="w-full mb-2 p-2 text-xs rounded border border-gray-200 h-20" />
            
            <div className="mt-2">
              <label className="text-[10px] font-black uppercase text-gray-400 block mb-1">QR Code Image</label>
              <input 
                type="file" 
                accept="image/*" 
                onChange={(e) => {
                  ...
                  if (file) {
                    ...
                    reader.onloadend = () => setJazzCashDetails({...jazzCashDetails, qrCodeBase64: reader.result as string});
                    reader.readAsDataURL(file);
                  }
                }} 
                className="text-[10px] w-full"
              />
              {jazzCashDetails.qrCodeBase64 && (
                <div className="mt-2 relative w-20 h-20 rounded border overflow-hidden">
                  <img src={jazzCashDetails.qrCodeBase64} alt="QR" className="w-full h-full object-cover" />
                  <button onClick={() => setJazzCashDetails({...jazzCashDetails, qrCodeBase64: ''})} className="absolute top-0 right-0 bg-red-500 text-white p-0.5 rounded-bl"><Trash2 size={10} /></button>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
            ...
            <input type="text" placeholder="Account Number" value={easypaisaDetails.accountNumber} onChange={e => setEasypaisaDetails({...easypaisaDetails, accountNumber: e.target.value})} className="w-full mb-2 p-2 text-xs rounded border border-gray-200" />
            <input type="number" placeholder="Max Deposit Limit" value={easypaisaDetails.maxDeposit} onChange={e => setEasypaisaDetails({...easypaisaDetails, maxDeposit: Number(e.target.value)})} className="w-full mb-2 p-2 text-xs rounded border border-gray-200" />
            <textarea placeholder="Instructions" value={easypaisaDetails.instructions} onChange={e => setEasypaisaDetails({...easypaisaDetails, instructions: e.target.value})} className="w-full mb-2 p-2 text-xs rounded border border-gray-200 h-20" />
            ...
            <div className="mt-2">
              ...
              <input 
                ...
                onChange={(e) => {
                  ...
                  if (file) {
                    const reader = new FileReader();
                    reader.onloadend = () => setEasypaisaDetails({...easypaisaDetails, qrCodeBase64: reader.result as string});
                    reader.readAsDataURL(file);
                  }
                }} 
                className="text-[10px] w-full"
              />
              {easypaisaDetails.qrCodeBase64 && (
                <div className="mt-2 relative w-20 h-20 rounded border overflow-hidden">
                  <img src={easypaisaDetails.qrCodeBase64} alt="QR" className="w-full h-full object-cover" />
                  <button onClick={() => setEasypaisaDetails({...easypaisaDetails, qrCodeBase64: ''})} className="absolute top-0 right-0 bg-red-500 text-white p-0.5 rounded-bl"><Trash2 size={10} /></button>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100">
            ...
            <input type="text" placeholder="Account Number" value={bankDetails.accountNumber} onChange={e => setBankDetails({...bankDetails, accountNumber: e.target.value})} className="w-full mb-2 p-2 text-xs rounded border border-gray-200" />
            <input type="number" placeholder="Max Deposit Limit" value={bankDetails.maxDeposit} onChange={e => setBankDetails({...bankDetails, maxDeposit: Number(e.target.value)})} className="w-full mb-2 p-2 text-xs rounded border border-gray-200" />
            <textarea placeholder="Instructions" value={bankDetails.instructions} onChange={e => setBankDetails({...bankDetails, instructions: e.target.value})} className="w-full mb-2 p-2 text-xs rounded border border-gray-200 h-20" />
            ...
            <div className="mt-2">
              ...
              <input 
                ...
                onChange={(e) => {
                  ...
                  if (file) {
                    const reader = new FileReader();
                    reader.onloadend = () => setBankDetails({...bankDetails, qrCodeBase64: reader.result as string});
                    reader.readAsDataURL(file);
                  }
                }} 
                className="text-[10px] w-full"
              ...
              {bankDetails.qrCodeBase64 && (
                <div className="mt-2 relative w-20 h-20 rounded border overflow-hidden">
                  <img src={bankDetails.qrCodeBase64} alt="QR" className="w-full h-full object-cover" />
                  <button onClick={() => setBankDetails({...bankDetails, qrCodeBase64: ''})} className="absolute top-0 right-0 bg-red-500 text-white p-0.5 rounded-bl"><Trash2 size={10} /></button>
                </div>
              )}
            </div>
          </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {pendingWithdrawals.map((w) => (
                  <tr key={w.id} className="hover:bg-gray-50 transition-colors">
                    ...
                    <td className="p-4 text-xs text-gray-600 max-w-[200px] truncate" title={w.accountDetails}>
                      {w.accountName ? `${w.accountName} (${w.accountNumber})` : w.accountDetails}
                    </td>
        ...
        <div className="grid lg:grid-cols-2 gap-6">
          {gameModes.map(mode => {
            ...
            const calculateAdminProfit = (resultNum: number) => {
              if (!summary) return 0;
              let totalPayout = 0;
              totalPayout += (summary.number[resultNum.toString()] || 0) * 8.1;
              const bigSmall = resultNum >= 5 ? 'Big' : 'Small';
              totalPayout += (summary.size[bigSmall] || 0) * 1.8;
              let colors = [];
              ...
              if (colors.includes('violet')) {
                totalPayout += (summary.color['violet'] || 0) * 4.05;
                if (colors.includes('red')) totalPayout += (summary.color['red'] || 0) * 1.35;
                if (colors.includes('green')) totalPayout += (summary.color['green'] || 0) * 1.35;
              } else {
                if (colors.includes('red')) totalPayout += (summary.color['red'] || 0) * 1.8;
                if (colors.includes('green')) totalPayout += (summary.color['green'] || 0) * 1.8;
              }
      ...
      <div className="bg-white rounded-[32px] shadow-sm border border-gray-200 p-8 mb-8">
        <h2 className="text-xl font-black mb-4 flex items-center gap-2 uppercase tracking-tight">
          <Zap className="text-orange-500" size={24} /> Global Settings
        </h2>
        
        <div className="mb-8">
          <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Minimum Withdrawal Limit (Rs)</label>
          <input
            type="number"
            value={minWithdrawal}
            onChange={(e) => setMinWithdrawal(Number(e.target.value))}
            className="w-full p-4 bg-gray-50 border border-gray-200 rounded-2xl font-black text-lg focus:outline-none focus:ring-2 focus:ring-green-500 transition-all mb-6"
            placeholder="200"
          />

          <label className="block text-xs font-black text-gray-400 uppercase tracking-widest mb-2">Win Probability ({winProbability}%)</label>
          <div className="flex items-center gap-4 p-4 bg-gray-50 rounded-2xl border border-gray-100">
            ...
            />
            <span className="font-black text-green-600 w-12 text-center">{winProbability}%</span>
          </div>
          <p className="text-[10px] text-gray-400 font-bold mt-2 uppercase tracking-tighter ml-1">
            Higher value = more users win. Lower value = more admin profit.
          </p>
        </div>
        ...
        <textarea
          ...
          className="w-full h-40 p-4 bg-gray-50 border border-gray-200 rounded-2xl font-mono text-sm mb-6 focus:outline-none focus:ring-2 focus:ring-green-500 transition-all"
          placeholder="<div style='background: #ff0; padding: 10px;'>Your Ad Here</div>"
        />

        <p className="text-xs text-gray-400 font-bold mb-4">Paste your "How to Play?" Video Embed Code (HTML) below.</p>
        <textarea
          value={howToPlayVideoHtml}
          onChange={(e) => setHowToPlayVideoHtml(e.target.value)}
        ...
        >
          {isSavingAd ? "Saving..." : "Save Global Settings"}
        </button>
      </div>
          ...
          >
            ...
            >
              ...
              <div className="space-y-4 text-sm">
                <p><strong>User:</strong> {viewingWithdrawal.userEmail}</p>
                <p><strong>Amount:</strong> Rs {viewingWithdrawal.amount}</p>
                <p><strong>Method:</strong> <span className="uppercase font-bold">{viewingWithdrawal.method}</span></p>
                
                {viewingWithdrawal.accountName && (
                  <p><strong>Account Name:</strong> {viewingWithdrawal.accountName}</p>
                )}
                {viewingWithdrawal.bankName && (
                  <p><strong>Bank Name:</strong> {viewingWithdrawal.bankName}</p>
                )}
                {viewingWithdrawal.accountNumber && (
                  <p><strong>Account Number:</strong> {viewingWithdrawal.accountNumber}</p>
                )}
                {!viewingWithdrawal.accountName && viewingWithdrawal.accountDetails && (
                  <p><strong>Account Details:</strong> {viewingWithdrawal.accountDetails}</p>
                )}
                
                <p><strong>Status:</strong> <span className="capitalize font-bold">{viewingWithdrawal.status}</span></p>
                <p><strong>Time:</strong> {new Date(viewingWithdrawal.timestamp).toLocaleString()}</p>

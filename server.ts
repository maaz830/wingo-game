...
// Initialize Client SDK for public settings access
      ...
      for (const mode of Object.keys(gameStates)) {
        ...
        if (elapsed >= gameStates[mode].duration) {
          ...
          if (gameStates[mode].forcedResult !== null) {
            ...
            // Fetch win probability from Firestore using Client SDK
            let winProb = 50;
            try {
              const settingsDoc = await getDoc(doc(clientDb, 'settings', 'global'));
              if (settingsDoc.exists()) {
                winProb = settingsDoc.data()?.winProbability ?? 50;
              } else {
                console.log("[SERVER] settings/global not found, using default winProb: 50");
              }
            } catch (e) {
              console.error("[SERVER] Error fetching winProbability with Client SDK:", e);
            }

            const numberPayouts = possibleNumbers.map(n => {
              ...
              betsToSettle.forEach(bet => {
                if (bet.type === 'number') {
                  if (bet.value === n) currentPayout += bet.amount * 8.1;
                } else if (bet.type === 'color') {
                  if (colors.includes(bet.value)) {
                    if (bet.value === 'violet') currentPayout += bet.amount * 4.05;
                    else if (n === 0 || n === 5) currentPayout += bet.amount * 1.35;
                    else currentPayout += bet.amount * 1.8;
                  }
                } else if (bet.type === 'size') {
                  if (bet.value === bigSmall) currentPayout += bet.amount * 1.8;
                }
              });
              return { n, payout: currentPayout };
            });

            // Sort by payout (ascending = admin profit, descending = user win)
            const sortedByPayout = [...numberPayouts].sort((a, b) => a.payout - b.payout);
            
            const rand = Math.random() * 100;
            if (rand < winProb) {
              // User win: Pick from the numbers with highest payouts
              // We pick from the top 3 to keep some randomness
              const top3 = [...numberPayouts].sort((a, b) => b.payout - a.payout).slice(0, 3);
              num = top3[Math.floor(Math.random() * top3.length)].n;
            } else {
              // Admin profit: Pick from the numbers with lowest payouts
              const bottom3 = sortedByPayout.slice(0, 3);
              num = bottom3[Math.floor(Math.random() * bottom3.length)].n;
            }
          }
          ...
          for (const bet of betsToSettle) {
            ...
            if (bet.type === 'number') {
              ...
              if (result.colors.includes(bet.value)) {
                won = true;
                if (bet.value === 'violet') {
                  winAmount = bet.amount * 4.05;
                } else {
                  // If result is 0 or 5, payout is 1.35x for Red/Green
                  if (result.number === 0 || result.number === 5) {
                    winAmount = bet.amount * 1.35;
                  } else {
